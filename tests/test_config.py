...
class TestCandidatePoolInvariants:
    ...
    def test_detects_pools_smaller_than_the_rerank_window(self, monkeypatch):
        cfg = reload_config(
            monkeypatch,
...
class TestEnvCoercion:
    ...
    def test_bool_parsing(self, monkeypatch, raw, expected):
        # An empty value is treated as unset, so the default (True) applies.
        monkeypatch.setenv("SOME_FLAG", raw)
        assert config._env_bool("SOME_FLAG", True) is expected
...
class TestLoggingSetup:
    def test_setup_logging_is_idempotent(self):
        ...
        config.setup_logging()  # must not raise or reconfigure
