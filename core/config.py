...
try:
    ...
    _bootstrap_logger.warning(
        "DB_PORT=%r is not an integer, falling back to 5432", _db_port_raw
    )
    DB_PORT = 5432

# Connection pool sizing. The pool is the hard limit on concurrent database
# work; see core.database for the semaphore that keeps callers from exceeding
# it (psycopg2 raises rather than queueing when the pool is exhausted).
